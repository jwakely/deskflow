      ...
      m_lblTitle{new QLabel(tr("Log"), this)}
{
  ...
  m_btnFloat->setIcon(QIcon::fromTheme(QIcon::ThemeIcon::ViewRestore));
  ...
  titleLayout->addWidget(m_lblTitle, Qt::AlignLeft);
  titleLayout->addSpacerItem(new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Fixed));
  titleLayout->addWidget(m_btnFloat, Qt::AlignRight);
  titleLayout->addWidget(m_btnClose, Qt::AlignRight);
